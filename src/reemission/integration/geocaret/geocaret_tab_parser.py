...
the GeoCARET reservoir and catchment delineation package.

The purpose of this module is to create a single output .csv file from mulitple 
csv output files generated by GeoCARET (e.g. from multiple analyses of different 
...
from reemission.integration.geocaret.custom_exceptions import \
...
# By default output csv file from GeoCARET is called 'output_parameters.csv'
...
@dataclass
class TabularGeoCaretOutput:
    """Collection of methods for pre-processing and validating tabular output
    data from GeoCARET prior to conversion into RE-EMISSION input file,"""
    data: pd.DataFrame

    def __add__(self, other: TabularGeoCaretOutput) -> TabularGeoCaretOutput:
        """Addition of parsers by concatenating their dataframe attributes.
        Used for working on multiple dataframes, e.g. tabular outputs from GeoCaret
        from multiple batches of reservoirs."""
        if other.__class__ == self.__class__:
            concat_data = pd.concat([self.data, other.data], axis=0, ignore_index=True)
            return TabularGeoCaretOutput(concat_data)
    ...
    def from_csv(
            ...
            id_column: Optional[str] = None) -> TabularGeoCaretOutput:
        """Instantiate object from csv file containing tabular GeoCaret output data
        """
...
class GeoCaretOutputReader(pydantic.BaseModel):
    """Reads individual tabular outputs from GeoCaret and combines the data
    ...
    def read_files(self) -> TabularGeoCaretOutput:
        ...
        file_parser = TabularGeoCaretOutput.from_csv(self.file_paths[0])
        # Combine files if more than one is provided
        if len(self.file_paths) > 1:
            for file_path in self.file_paths[1:]:
                file_parser = file_parser + TabularGeoCaretOutput.from_csv(file_path)
        return file_parser
