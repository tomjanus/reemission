...
<<<<<<< HEAD
from .dask_utils import dask_util 
# Support for labelled multi-dimensional arrays
import xarray as xr
=======
import xarray
>>>>>>> df59a6922f0090969ff310d2f4810237264fd1e1
...
<<<<<<< HEAD
"""instantiate dask cluster to be used in later processes"""
dask_instance = dask_util
dask_instance.init_cluster(dask_instance)
...
class Dem(Layer):
    ...
    def data(self) -> RasterioRaster:
        ...
        if self._data is None:
            self.load()
        return self._data
    def raster_attr(self, new_path: str, ref_data: str)->None:
        """set metdata for raster file since whiteboxtool
        does not provide it in its results""" 
        dataset = self.data 
        ref_data = Dem(path=ref_data)
        ref_dataset = ref_data.data 
        profile = dataset.profile 
        target = dataset.read(
            out_shape=(
                dataset.count,
                int(dataset.height * 1),
                int(dataset.width * 1)
        ...
        new_height = int(dataset.height * 1)
        new_width = int(dataset.width * 1)
        profile.update(transform=transform, driver='GTiff',
                       height=new_height, width=new_width,
                       crs=ref_dataset.crs)
        new_file = os.path.join(new_path,'dem_downscaled.tif')
...
<<<<<<< HEAD
        ...
        #dask_client = dask_init() #Client(n_workers = n_workers)
        dask_instance.dask_check(dask_instance)
...
>>>>>>> df59a6922f0090969ff310d2f4810237264fd1e1
        ...
        if not parallel:
            for zone in zones.collection:
                self.clip_to_polygon(zone=zone,
                                                    clipped_path=
                                                    clipped_path,
                                                    parallel=parallel,
